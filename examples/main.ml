...
let parse_module enable_type_comment filename =
  ...
  handle_result result ~f:(fun ast ->
    let s = Stats.py_module Stats.State.default ast in
    let ast = Classtransforms.py_module s ast in
...
let main_cmd =
  ...
  let info = Cmd.info "opine-examples" ~version:"dev" ~doc ~sdocs ~man:help_sections in
